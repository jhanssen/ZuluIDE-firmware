...
class AudioReadCallback;
static AudioReadCallback* callback_buf_a = nullptr;
static AudioReadCallback* callback_buf_b = nullptr;
...
#define AUDIO_WIRE_BUFFER_SIZE (SAMPLE_CHUNK_SIZE * 2)
static uint16_t wire_buf_a[AUDIO_WIRE_BUFFER_SIZE];
static uint16_t wire_buf_b[AUDIO_WIRE_BUFFER_SIZE];

// tracking for audio playback
static uint8_t audio_owner; // SCSI ID or 0xFF when idle
static volatile bool audio_paused = false;
static IDEImageFile* audio_file;
static uint64_t fpos;
static uint32_t fleft;

// historical playback status information
static audio_status_code audio_last_status[8] = {
...
class AudioReadCallback : public IDEImage::Callback
{
public:
    AudioReadCallback(uint8_t* data)
        : mData(data)
    {
    }
...
private:
    virtual ssize_t read_callback(const uint8_t *data, size_t blocksize, size_t num_blocks) override;
    virtual ssize_t write_callback(uint8_t *data, size_t blocksize, size_t num_blocks, bool first_xfer, bool last_xfer) override;
...
ssize_t AudioReadCallback::read_callback(const uint8_t *data, size_t blocksize, size_t num_blocks)
{
    mLastRead = blocksize * num_blocks;
    memcpy(mData, data, mLastRead);
    return static_cast<ssize_t>(mLastRead);
}

ssize_t AudioReadCallback::write_callback(uint8_t *, size_t, size_t, bool, bool)
{
    assert("AudioReadCallback should only read" && false);
    return 0;
}
...
void audio_poll() {
    ...
    AudioReadCallback* audiocb;
    if (sbufst_a == STALE) {
        sbufst_a = FILLING;
        audiocb = callback_buf_a;
    } else if (sbufst_b == STALE) {
        sbufst_b = FILLING;
        audiocb = callback_buf_b;
    } else {
    ...
    if (!audio_file->read(fpos, 1, toRead, audiocb) || audiocb->lastRead() != toRead) {
...
bool audio_play(uint8_t owner, IDEImageFile* img, uint64_t start, uint64_t end, bool swap) {
    ...
    platform_set_sd_callback(NULL, NULL);
    if (callback_buf_a == nullptr) {
        callback_buf_a = new AudioReadCallback(sample_buf_a);
    }
    if (callback_buf_b == nullptr) {
        callback_buf_b = new AudioReadCallback(sample_buf_b);
    }
    audio_file = img;
    ...
    uint64_t len = audio_file->capacity();
    if (start > len) {
    ...
    if (!audio_file->read(start, 1, AUDIO_BUFFER_SIZE, callback_buf_a) || callback_buf_a->lastRead() != AUDIO_BUFFER_SIZE) {
        logmsg("File playback start returned fewer bytes than allowed");
        return false;
    }
    if (!audio_file->read(start + AUDIO_BUFFER_SIZE, 1, AUDIO_BUFFER_SIZE, callback_buf_b) || callback_buf_b->lastRead() != AUDIO_BUFFER_SIZE) {
        logmsg("File playback start returned fewer bytes than allowed");
        return false;
    }

    // prepare initial tracking state
    fpos = audio_file->file_position();
    ...
    for (uint16_t i = 0; i < AUDIO_WIRE_BUFFER_SIZE; i++) {
        wire_buf_a[i] = 0;
        wire_buf_b[i] = 0;
    }
    sfcnt = 0;
    ...
    dma_channel_configure(SOUND_DMA_CHA, &snd_dma_a_cfg, &(spi_get_hw(AUDIO_SPI)->dr),
                          &wire_buf_a, AUDIO_WIRE_BUFFER_SIZE, false);
    ...
    dma_channel_configure(SOUND_DMA_CHB, &snd_dma_b_cfg, &(spi_get_hw(AUDIO_SPI)->dr),
                          &wire_buf_b, AUDIO_WIRE_BUFFER_SIZE, false);
...
void audio_stop(uint8_t id) {
    ...
    delete callback_buf_a;
    callback_buf_a = nullptr;
    delete callback_buf_b;
    callback_buf_b = nullptr;
}
